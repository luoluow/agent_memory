#!/usr/bin/env bash
# Claude Code Auto-Memory + dreaming consolidation eval on MeME filler32k (pl + sw), then judge.
set -euo pipefail
cd "$(dirname "$0")/.."
source .venvs/baseline_env/bin/activate
OUT=../../output/auto_memory_dreaming/claude-code
mkdir -p "$OUT"
for DOMAIN in pl sw; do
  echo "=== INGEST+ANSWER: $DOMAIN ==="
  python -m eval.run_agent -d "data/filler32k_${DOMAIN}" -o "$OUT" \
    --agent-type auto_memory_dreaming --model claude-code -w 1 --skip-existing
done
echo "=== JUDGE ==="
