"""Load MEME filler32k episodes as dspy.Example for SIMBA optimization.

Stratified split: balance PL and SW in train/test.
"""
import json
import random
...
def build_example(episode: Dict, domain: str) -> dspy.Example:
    """One Example = one episode.

    DSPy Example fields:
      input: episode dict (sessions + questions)
      output: expected answers for U-check
    """
...
def load_trainset_testset(
    ...
    n_test_per_domain: int = 15,
    seed: int = 42,
) -> tuple[List[dspy.Example], List[dspy.Example]]:
    """Stratified random split over filler32k episodes.
...
if __name__ == "__main__":
    ...
    train, test = load_trainset_testset(data_dir)
    print(f"Train: {len(train)} episodes")
    for ex in train:
        print(f"  {ex.domain} ep{ex.episode_id} ({len(ex.sessions)} sessions)")
    print(f"Test: {len(test)} episodes")
    for ex in test[:5]:
        print(f"  {ex.domain} ep{ex.episode_id}")
    print(f"  ...")
